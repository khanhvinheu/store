							...
							<form>
								<table class="table table-bordered .table-responsive text-center">
									...
									<tr>
										...
										<td><img class="img-responsive" height="150px" src="{{asset('lib/storage/app/avatar/'.$cart->attributes->img) }}"></td>
										<td>{{ $cart->name }}</td>
										<td>
											<div class="form-group">
												<input class="form-control" type="number" onchange="updateCart(this.value,{{ $cart->id }})" value="{{ $cart->quantity }}">
											</div>
										</td>
										...
										<td><a href="{{ asset('cart/delete/'.$cart->id )}}">Xóa</a></td>										
									</tr>
								@endforeach	
								</table>
